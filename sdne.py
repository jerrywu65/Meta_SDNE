...
class SDNE_BINARY_LOSS(object):
    ...
    def train(self):
        ...
        # L_2nd = tf.reduce_sum(tf.square((AdjBatch - fc) * B))
        L_2nd=tf.nn.sigmoid_cross_entropy_with_logits(logits=AdjBatch,labels=fc)
...
class SDNE_META_PATH(object):
    def __init__(self, graph, encoder_layer_list, alpha=1e-6, beta=5., nu1=1e-5, nu2=1e-4,
        ...
        self.meta_path_matrix=meta_path()
    ...
    def train(self):
        adj_mat = self.adj_mat
        meta_path_matrix=self.meta_path_matrix

        AdjBatch = tf.placeholder(tf.float32, [None, self.node_size], name='adj_batch')
        Adj = tf.placeholder(tf.float32, [None, None], name='adj_mat')
        B = tf.placeholder(tf.float32, [None, self.node_size], name='b_mat')
        Meta_matrix=tf.placeholder(tf.float32, [None, None], name='meta_path_matrix')
        ...
        #     )
        # )
        tmp1=tf.sigmoid(tf.matmul(_embeddings,tf.transpose(_embeddings)))
        tmp2=tf.sigmoid(Meta_matrix)
        L_1st=tf.sigmoid_cross_entropy_with_logits(logits=tmp1,labels=tmp2)
        ...
        for step in range(self.max_iter):
            ...
            meta_matrix_batch=meta_path_matrix[index,:]
            meta_matrix_train=meta_matrix_batch[:,index]

            self.sess.run(train_op, feed_dict={AdjBatch: adj_batch_train,
                                               Adj: adj_mat_train,
                                               B: b_mat_train,
                                               Meta_matrix:meta_matrix_train})
            if step % 50 == 0:
                l, l1, l2 = self.sess.run((L, L_1st, L_2nd),
                                          feed_dict={AdjBatch: adj_batch_train,
                                                     Adj: adj_mat_train,
